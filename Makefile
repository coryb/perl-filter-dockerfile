# This Makefile is for the Filter::Dockerfile extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/Filter/Dockerfile.pm]
#     AUTHOR => [q[Cory Bennett <cpan@corybennett.org>]]
#     BUILD_REQUIRES => { Test::More=>q[0] }
#     CONFIGURE_REQUIRES => { ExtUtils::MakeMaker=>q[0] }
#     EXE_FILES => [q[bin/dfpp]]
#     LICENSE => q[artistic_2]
#     MIN_PERL_VERSION => q[5.006]
#     NAME => q[Filter::Dockerfile]
#     PL_FILES => {  }
#     PREREQ_PM => { Test::More=>q[0], Filter::Util::Call=>q[0], LWP::UserAgent=>q[0], Mozilla::CA=>q[0] }
#     VERSION_FROM => q[lib/Filter/Dockerfile.pm]
#     clean => { FILES=>q[Filter-Dockerfile-*] }
...
H_FILES  = 
MAN1PODS = bin/dfpp
MAN3PODS = lib/Filter/Dockerfile.pm
...
TO_INST_PM = lib/Filter/Dockerfile.pm

PM_TO_BLIB = lib/Filter/Dockerfile.pm \
...
DISTNAME = Filter-Dockerfile
DISTVNAME = Filter-Dockerfile-0.0.1
...
manifypods : pure_all  \
	bin/dfpp \
	lib/Filter/Dockerfile.pm
	$(NOECHO) $(POD2MAN) --section=1 --perm_rw=$(PERM_RW) \
	  bin/dfpp $(INST_MAN1DIR)/dfpp.$(MAN1EXT) 
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/Filter/Dockerfile.pm $(INST_MAN3DIR)/Filter::Dockerfile.$(MAN3EXT) 
...
$(INST_SCRIPT)/dfpp : bin/dfpp $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/dfpp
	$(CP) bin/dfpp $(INST_SCRIPT)/dfpp
	$(FIXIN) $(INST_SCRIPT)/dfpp
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/dfpp
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''Dockerfile preprocessor'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '  - '\''Cory Bennett <cpan@corybennett.org>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: Filter-Dockerfile' >> META_new.yml
	$(NOECHO) $(ECHO) 'no_index:' >> META_new.yml
	$(NOECHO) $(ECHO) '  directory:' >> META_new.yml
	$(NOECHO) $(ECHO) '    - t' >> META_new.yml
	$(NOECHO) $(ECHO) '    - inc' >> META_new.yml
	$(NOECHO) $(ECHO) 'requires:' >> META_new.yml
	$(NOECHO) $(ECHO) '  Filter::Util::Call: '\''0'\''' >> META_new.yml
	$(NOECHO) $(ECHO) '  LWP::UserAgent: '\''0'\''' >> META_new.yml
	$(NOECHO) $(ECHO) '  Mozilla::CA: '\''0'\''' >> META_new.yml
	$(NOECHO) $(ECHO) '  perl: '\''5.006'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'version: v0.0.1' >> META_new.yml
	-$(NOECHO) $(MV) META_new.yml $(DISTVNAME)/META.yml
	$(NOECHO) $(ECHO) Generating META.json
	$(NOECHO) $(ECHO) '{' > META_new.json
	$(NOECHO) $(ECHO) '   "abstract" : "Dockerfile preprocessor",' >> META_new.json
	$(NOECHO) $(ECHO) '   "author" : [' >> META_new.json
	$(NOECHO) $(ECHO) '      "Cory Bennett <cpan@corybennett.org>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "Filter-Dockerfile",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '         "requires" : {' >> META_new.json
	$(NOECHO) $(ECHO) '            "Filter::Util::Call" : "0",' >> META_new.json
	$(NOECHO) $(ECHO) '            "LWP::UserAgent" : "0",' >> META_new.json
	$(NOECHO) $(ECHO) '            "Mozilla::CA" : "0",' >> META_new.json
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Dockerfile preprocessor</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Cory Bennett &lt;cpan@corybennett.org&gt;</AUTHOR>' >> $(DISTNAME).ppd
	...
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Mozilla::CA" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/Filter/Dockerfile.pm blib/lib/Filter/Dockerfile.pm 
